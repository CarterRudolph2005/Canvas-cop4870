...
namespace Canvas.MAUI.Views
{
    public partial class GradeSubmissionView : ContentPage
    {
        ...
        private void StudentPickerChanged(object sender, EventArgs e)
        {
            ...
            SubmissionText.Text = submission.Content ?? "No text submission.";
            GradeEntry.Text = submission.PointsAwarded?.ToString() ?? string.Empty;
            ViewModel.LoadComments(submission.Id);
            ViewModel.SelectedSubmissionFilePath = submission.FilePath;
            ViewModel.SelectedSubmissionMimeType = submission.MimeType;
        }

        private async void OpenFileClicked(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(ViewModel.SelectedSubmissionFilePath)) return;
            var url = $"http://localhost:5258{ViewModel.SelectedSubmissionFilePath}";
